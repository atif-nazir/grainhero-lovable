...
export default function SilosPage() {
  ...
  const fetchSilos = async () => {
    try {
      setLoading(true)
      const { data: silosData, error } = await supabase
        .from('silos')
        .select(`
          id,
          silo_id,
      ...
      const mappedSilos = (silosData || []).map(s => ({
        _id: s.id,
        silo_id: s.silo_id || s.id,
        name: s.name,
      ...
      setSilos(mappedSilos as unknown as Silo[]);
    } catch (error) {
  ...
  const fetchWarehouses = async () => {
    try {
      ...
      const warehouseData = (data || []).map(w => ({
        _id: w.id,
        warehouse_id: w.id,
        name: w.name
      }));
  ...
  const handleAddSilo = async () => {
    try {
      const dataToSend = {
        ...
        name: formData.name || `Silo-${Date.now().toString().slice(-4)}`,
        silo_id: formData.silo_id || `S-${Date.now().toString().slice(-4)}`
      }

      const { data, error } = await supabase.from('silos').insert(dataToSend).select().single()
  ...
  const handleEditSilo = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('silos').update(dataToSend).eq('id', selectedSilo._id)
  ...
  const handleDeleteSilo = async () => {
    if (!selectedSilo) return
    try {
      const { error } = await supabase.from('silos').delete().eq('id', selectedSilo._id)
      if (error) throw error;
